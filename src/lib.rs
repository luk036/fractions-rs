//! Fraction numbers
//!
//! ## Compatibility
//!
//! The `fractions-rs` crate is tested for rustc 1.31 and greater.

// #![no_std]
// Fraction ops often use other "suspicious" ops
#![allow(clippy::suspicious_arithmetic_impl)]
#![allow(clippy::suspicious_op_assign_impl)]

pub mod fractions;
pub use fractions::{const_abs, const_gcd, Fraction};

#[cfg(feature = "std")]
pub mod logging;

use core::ops::{Add, Mul, Sub};

/// The function `archimedes` calculates the area of a triangle using Archimedes' formula with the
/// lengths of the three sides provided as `Fraction<i64>` values.
///
/// Arguments:
///
/// * `side_a`: Represents the length of the first side of the triangle.
/// * `side_b`: The parameters `side_a`, `side_b`, and `side_c` represent the lengths of the sides of a triangle. In
///   the context of Archimedes' formula for the area of a triangle, `side_a`, `side_b`, and `side_c`
/// * `side_c`: The parameter `side_c` represents the length of the third side of the triangle.
///
/// Returns:
///
/// The function `archimedes` returns the area of a triangle computed using Archimedes' formula, given
/// the lengths of the 3 sides.
///
/// # Examples
///
/// ```
/// use fractions::{Fraction, archimedes};
///
/// let q_1 = Fraction::<i32>::new(1, 2);
/// let q_2 = Fraction::<i32>::new(1, 4);
/// let q_3 = Fraction::<i32>::new(1, 6);
/// let result = archimedes(&q_1, &q_2, &q_3);
/// assert_eq!(result, Fraction::<i32>::new(23, 144));
/// ```
#[inline]
pub fn archimedes<T>(side_a: &T, side_b: &T, side_c: &T) -> T
where
    T: Copy + Add<Output = T> + Sub<Output = T> + Mul<Output = T> + From<i32>,
{
    let temp_sum = *side_a + *side_b - *side_c;
    T::from(4) * *side_a * *side_b - temp_sum * temp_sum
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_archimedes4() {
        let side_a = Fraction::<i32>::new(1, 2);
        let side_b = Fraction::<i32>::new(1, 4);
        let side_c = Fraction::<i32>::new(1, 6);
        assert_eq!(
            archimedes(&side_a, &side_b, &side_c),
            Fraction::<i32>::new(23, 144)
        );
    }

    #[test]
    fn test_archimedes5() {
        let side_a = Fraction::<i64>::new(1, 2);
        let side_b = Fraction::<i64>::new(1, 4);
        let side_c = Fraction::<i64>::new(1, 6);
        assert_eq!(
            archimedes(&side_a, &side_b, &side_c),
            Fraction::<i64>::new(23, 144)
        );
    }
}

#[cfg(test)]
mod more_tests;
